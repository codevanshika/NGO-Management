...
public class LoginCtl extends BaseCtl {
	...
	protected boolean validate(HttpServletRequest request) {
		...
		if (DataValidator.isNull(login)) {
			 ...
			 * PropertyReader.getValue("error.email", "Login Id ")); pass = false; }
			 */
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (id > 0) {
			...
			ServletUtility.forward(NGOView.LOGIN_VIEW, request, response);
			return;
		}
		if (session.getAttribute("user") != null) {
			ServletUtility.redirect(NGOView.WELCOME_CTL, request, response);
			return;
		}
		...
		if (OP_SIGN_IN.equalsIgnoreCase(op)) {
			...
			try {
				...
				if (bean != null) {
					...
					//if (roleBean != null) {                                        I COMMENTED THIS IF BLOCK
			...
			ServletUtility.redirect(NGOView.USER_REGISTRATION_CTL, request, response);
		return;
		}
		ServletUtility.forward(getView(), request, response);
